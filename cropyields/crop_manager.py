...
class SingleRotationAgroManager(YAMLAgroManagementReader):
    ...
    def retrieve_year(self):
        """Retrieve year in which crop is grown"""
        calendar_years = [
            date.year
    ...
    def retrieve_variety(self):
        """Retrieve crop variety"""
        variety_names = [
            item[date_key]["CropCalendar"]["variety_name"]
            if item[date_key]["CropCalendar"]
            else None
            for item in self
        ...
        for item in self:
            ...
            for key, value in item.items():
                if isinstance(key, dt.date):
                    ...
                    modified_item[new_date] = self._recursively_change_year(
                        value, year_increment
                    )
                else:
                    modified_item[key] = self._recursively_change_year(
                        value, year_increment
                    )
        ...
        if isinstance(obj, dict):
            ...
            for k, v in obj.items():
                ...
                if isinstance(v, (dict, list)):
                    modified_dict[new_key] = self._recursively_change_year(
                        v, year_increment
                    )
    ...
    def change_variety(self, new_variety):
        ...
        for item in self:
            for date_key in item:
                if (
                    "CropCalendar" in item[date_key]
                    and item[date_key]["CropCalendar"] is not None
                ):
...
class Crop:
    ...
    :param 'TimedEvents': list of events defined by their timing
    :param 'StateEvents': list of events depending on phenology rather than
        ...
        if self.crop.lower() != "fallow":
            if "crop_start_date" not in args:
                if "crop_start_month" not in args and "crop_start_day" not in args:
                    raise ValueError(
                        f"Please provide a crop start date for "
                        f"{self.crop} as kwargs['crop_start_day']"
                        f" and kwargs['crop_start_month'] or kwargs"
                        f"['crop_start_date']"
                    )
                else:
            ...
            if "start_crop_calendar" not in args:
                if "crop_start_month" not in args and "crop_start_day" not in args:
                    raise ValueError(
                        "Please provide a calendar start date for "
                        "'fallow' as kwargs['start_crop_calendar']"
                        " or kwargs['crop_start_day'] and kwargs"
                        "['crop_start_month']"
                    )
            else:
        ...
        if "winter" in variety.lower():
            ...
            if "date" not in event:
                ...
                if timing < crop_start_date:
                    # this deals with a crop calendar year starting
                    # in the fall and timing events in the following year
